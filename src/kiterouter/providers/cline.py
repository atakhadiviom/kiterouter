"""Cline provider: Anthropic & OpenRouter adapter for Cline agent."""
...
class ClineProvider(BaseProvider):
    ...
    async def stream_chat(
        ...
        **kwargs: Any,
        ...
        if not self.api_key:
            yield create_sse_chunk("Cline error: Set CLINE_API_KEY or OPENROUTER_API_KEY.", model=model)
            yield "data: [DONE]\n\n"
            return
        ...
        headers = {
            ...
            "HTTP-Referer": "https://kiterouter.dev",
            "X-Title": "KiteRouter-Cline",
        }
        payload = {
            "model": model,
            "messages": messages,
